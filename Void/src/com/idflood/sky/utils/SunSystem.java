...
public class SunSystem {
    ...
    private static float distScaleFactor = 0x384;
    double lambda;
    double beta;
    double r;
    double lambdaOffset;
    ...
    private Vector3f calculateCartesianCoords(Date date) {
        ...
        int MM = gc.get(Calendar.MONTH) + 0x01;
        ...
        matRx.fromAngleNormalAxis((float) -eta, new Vector3f(0x01, 0x00, 0x00));
        matRy.fromAngleNormalAxis((float) -(lat - Math.PI / 2), new Vector3f(0x00,
                        0x01, 0x00));
        matRz.fromAngleNormalAxis((float) -LMST, new Vector3f(0x00, 0x00, 0x01));
